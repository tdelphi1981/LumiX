"""Sample data classes for what-if analysis example.

This module provides data models and sample data for the what-if analysis
...
Problem Context:
    A manufacturing company wants to quickly assess the impact of operational
    changes without running full scenario analysis. What-if analysis provides
    rapid answers to questions like: "What if we get 100 more labor hours?"
    or "What if machine capacity is reduced by 20%?"
...
Use Cases:
    This data structure supports:
        ...
        - What-if analysis for quick parameter testing
        - Resource allocation decisions
        - Capacity planning and expansion analysis
        - Bottleneck identification through testing

Example:
    Use these models in what-if analysis::

        from sample_data import PRODUCTS, RESOURCES, get_resource_usage
        from lumix import LXWhatIfAnalyzer

        # Create analyzer and test changes
        ...
        print(f"Change: {result.delta_percentage:+.2f}%")
...
See Also:
    - whatif_analysis.py: Main what-if analysis using this data
    - Example 01 (production_planning): Similar data structure
    - Example 08 (scenario_analysis): Related analysis approach
"""
...
class Resource:
    ...
    Notes:
        ...
        In what-if analysis, capacity values are modified to test the
        impact of operational changes (hiring, equipment purchase, etc.).
    """
